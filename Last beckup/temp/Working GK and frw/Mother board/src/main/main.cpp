	...
	while(1)
	{
		...
		if(switchPin.readPin())
		{
			...
			if(CODE == ATTACKER) {
				...
				//STABLE PART
				if (tsop.isCanSee()) {
					if(abs(cam.getCamDist() * cos(cam.getCamAngle() / 57.3)) > 50 || abs(cam.getCamDist() * sin(cam.getCamAngle() / 57.3)) < 30) {
						if (tsop.getDist() > 40) {
						...
						if (tsop.getDist() > 40) {
							if (abs(float(tAng)) > 15) {
								if (abs(float(tAng)) < 60) {
									math.setVector(tsop.getAngle() + (tAng > 0 ? 60 : -60), 2000);
								} else {
				...
				//Line detection
				/*if(abs(cam.getCamDist() * cos(cam.getCamAngle() / 57.3)) < 80) 
				{
				...
				if(abs(cam.getCamDist() * cos(cam.getCamAngle() / 57.3)) < 80) 
				{
					if(cam.getCamDist() * sin(cam.getCamAngle() / 57.3) > 50)// || cam.getAnotherCamDist() * sin(cam.getAnotherCamAngle() / 57.3) > 50)	
					{
						math.addVector(90, math.getSpeed() + speed);
					}
					if(cam.getCamDist() * sin(cam.getCamAngle() / 57.3) < -50)// || cam.getAnotherCamDist() * sin(cam.getAnotherCamAngle() / 57.3) < -50)	
					{
						math.addVector(-90, math.getSpeed() + speed);
				...
				{
					math.addVector(180, math.getSpeed() + speed);
				}
				//
				...
				if(tsop.isCanSee()) {
					if ((abs(float(tAng)) > 140 || abs(float(tsop.getAngle())) < 45) && tsop.getDist() > 18) { 
						if (gkGo == 0) { 
							gkGo = 1;
							timer = millis();
						}
						...
						if (gkGo == 2 && millis() - timer < 1500) { 
							...
							//		math.setVector(cam.getAnotherCamAngle(), 3500);
							//else {
								if (abs(float(tsop.getAngle())) < 25) {
										math.setVector(0, 3000);
								} else {
										math.setVector(tsop.getAngle() * 1.5, 3000);
								}
							//}
						}
						if (gkGo == 2 && millis() - timer >= 1500) { 
				...
				if (gkGo != 2) {
					...
					if (abs(cam.getCamDist() * sin(cam.getCamAngle() / 57.3)) > 45) {
						gkGo = 0;
					}
				...
				if (abs(float(tsop.getAngle())) > 160) {
							math.setVector(0, 0);
				}
				math.calculateSpeed(math.getAngle(), math.getSpeed(), sp1, sp2, sp3, sp4);
				k = gyro.getAngle();
